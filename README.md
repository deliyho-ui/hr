...
הקוד לא משתמש בחבילת `openai`. הוא שולח בקשת HTTP רגילה אל endpoint תואם Responses API. ברירת המחדל היא:

```txt
AI_RESPONSES_URL=https://api.openai.com/v1/responses
```

ברוב המקרים אין צורך להגדיר את `AI_RESPONSES_URL`. אם עובדים עם endpoint אחר שתואם לאותו מבנה בקשה/תגובה, אפשר להגדיר אותו ב־Vercel. קיימים גם שמות גיבוי למפתח ולמודל: `AI_API_KEY`, `OPENAI_API_KEY`, `CODEX_MODEL`, `OPENAI_MODEL`.

...
3. לפרסם את `firestore.rules` **מריפו זה** (מקור האמת לכל הפרויקט, כולל דף הנחיתה)
4. לפרסם את `storage.rules` **מריפו זה**

כרטיס המועמד מציג את כל שדות השאלון מדף הנחיתה (`answers` + פרטים אישיים). היסטוריית שינויים נשמרת עם `arrayUnion` כדי למנוע גידול מיותר של המסמך.
